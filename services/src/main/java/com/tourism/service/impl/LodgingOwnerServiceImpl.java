...
public class LodgingOwnerServiceImpl implements LodgingOwnerService {
   ...
   public Either<ErrorDto[], LodgingOwner> delete(UUID id) {
      try {
         ...
         log.error(e.getMessage());
         return Either.left(
               new ErrorDto[] { ErrorDto.of(HttpStatus.INTERNAL_SERVER_ERROR, MessageConstants.ERROR_DELETING_LODGING_OWNER, e.getMessage()) });
      }
   }
   ...
   public Either<ErrorDto[], LodgingOwnerResponseDTO> getById(UUID id) {
      try {
         ...
         log.error(e.getMessage());
         return Either.left(
               new ErrorDto[] { ErrorDto.of(HttpStatus.INTERNAL_SERVER_ERROR, MessageConstants.ERROR_GET_LODGING_OWNER, e.getMessage()) });
      }
   }
