...
public class AuthServiceImpl implements AuthService {
   ...
   public Either<ErrorDto[], Map<String, String>> login(AuthUserDto authUserDto) {
      try {
         ...
         if (user.isEmpty()) {
            ...
            return Either.left(new ErrorDto[] {
                  new ErrorDto(HttpStatus.BAD_REQUEST, ErrorsCode.LOGIN_FAILED.name(), MessageConstants.ERROR_INCORRECT_USER_OR_PASSWORD) });
         }

         if (encryptionService.checkPassword(authUserDto.getPassword(), user.get().getPassword())) {
   ...
   public Either<ErrorDto[], Boolean> logout(HttpServletRequest request) {
      ...
      if (refreshToken != null && jwtTokenProvider.validateRefreshToken(refreshToken)) {
         ...
         return Either.left(
               new ErrorDto[] { new ErrorDto(HttpStatus.BAD_REQUEST, ErrorsCode.LOGOUT_FAILED.name(), MessageConstants.ERROR_INVALID_TOKEN) });
      }
   }
   ...
   public Either<ErrorDto[], String> refreshToken(HttpServletRequest request) {
      ...
      if (refreshToken != null && jwtTokenProvider.validateRefreshToken(refreshToken)) {
         ...
         if (token != null) {
            ...
            return Either.left(new ErrorDto[] {
                  new ErrorDto(HttpStatus.BAD_REQUEST, ErrorsCode.INVALID_REFRESH_TOKEN.name(), MessageConstants.ERROR_REFRESH_TOKEN_NOT_FOUND) });
         }
      } else {
