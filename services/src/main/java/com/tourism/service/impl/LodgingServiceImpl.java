...
public class LodgingServiceImpl implements LodgingService {
   ...
   public Either<ErrorDto[], LodgingResponseDTO> create(LodgingRequestDTO requestDto, UUID ownerId) {
      try {
         TouristicPlace place = placeRepository
               .findById(requestDto.touristicPlaceId())
               .orElseThrow(() -> new EntityNotFoundException("Touristic Place not found with id: " + requestDto.touristicPlaceId()));
         LodgingOwner owner = ownerRepository
               .findById(ownerId)
               .orElseThrow(() -> new EntityNotFoundException("Lodging Owner not found with id: " + ownerId));
   ...
   public Either<ErrorDto[], Lodging> delete(UUID id) {
      try {
         ...
         log.error(e.getMessage());
         return Either.left(
               new ErrorDto[] { ErrorDto.of(HttpStatus.NOT_FOUND, MessageConstants.ERROR_DELETING_TOURISTIC_PLACE, "Touristic Place not found") });
         ...
         log.error(e.getMessage());
         return Either.left(
               new ErrorDto[] { ErrorDto.of(HttpStatus.INTERNAL_SERVER_ERROR, MessageConstants.ERROR_DELETING_LODGING, e.getMessage()) });
      }
   }
