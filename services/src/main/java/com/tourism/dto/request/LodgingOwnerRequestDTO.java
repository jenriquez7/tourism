...
public class LodgingOwnerRequestDTO {

    @Schema(example = "John", description = "owner first name")
    @Size(max = 100, message = "{validation.name.size.too_long}")
    @NotNull
    private String firstName;

    @Schema(example = "Doe", description = "owner last name")
    @Size(max = 100, message = "{validation.name.size.too_long}")
    @NotNull
    private String lastName;

    @Schema(example = "example@mail.com", description = "owner email address")
